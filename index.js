...
const OWNER_ID = 5740663438; 
const REQUIRED_CHANNEL = '@acafh'; 
...
function showWelcomeBanner() {
 ...
 ▀▀▀▀ ▀─▀ ▀▀▀▀ ▀▀▀▀ ▀─▀ ▀▀▀▀ ▀─▀▀ 
 ───────────────────────────────────${reset}`);
    console.log(`${cyan}${bold} WELCOME OWNER ACA STORE - WINDOWS LOCAL SYSTEM${reset}`);
...
function logToPanel(type, data) {
    ...
    if (type === 'SATUAN') {
        ...
        console.log(`${magenta}${bold}└── [ACA LOG TRACKER SYSTEM] ──────────────────────────┘${reset}\n`);
    } else if (type === 'BULK') {
        ...
        console.log(`${cyan}${bold}└── [ACA LOG TRACKER SYSTEM] ──────────────────────────┘${reset}\n`);
    }
}
...
bot.start(async (ctx) => {
  ...
  `terdapat 3 fitur convert yang tersedia :\n1. convert link to new link\n 2. convert link to cookies\n 3. convert cookies to link\n\n` +
        ...
        `• Owner Bot : @mubffle</blockquote>\n` +
        `<b>Silakan pilih menu di bawah untuk memulai operasional.</b>`;

    ctx.replyWithPhoto('https://cdn.phototourl.com/free/2026-08-06-64cb4f23-e19f-429e-996c-ab074e9b09a8.jpg', {
        caption: welcomeText,
        parse_mode: 'HTML',
...
bot.on('text', async (ctx) => {
    ...
    await ctx.replyWithPhoto('https://cdn.phototourl.com/free/2026-08-06-64cb4f23-e19f-429e-996c-ab074e9b09a8.jpg', {
        caption: cardLayout,
        parse_mode: 'HTML',
...
bot.action('btn_leaderboard', async (ctx) => {
    ...
    let textLeaderboard = `<b>🏆 TOP 10 RANKING CONVERTER ACA STORE</b>\n─────────────────────────────\n`;
